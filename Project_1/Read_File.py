...
def read_file(filename):
	...
	with open("./Results/" + filename, 'r') as infile:
		...
		print ("Yey")
		plt.figure()
		plt.plot(x, Approx_General, "-k", label="$v(x) - Numerical$", linewidth=3)
		plt.plot(x, Exact, "--r", label="$u(x) - Analytical$")
		plt.title("Numerical and analytical soulutions with %d datapoints." % n,fontsize=13)
